<template>
    <transition name="fade-zoom">
        <div v-if="visible" class="fm-panel-shell" :style="{
            ...
            top: panel.y + 'px',
            zIndex: panelZ
        }" role="dialog" aria-label="文件管理面板" tabindex="0" @mousedown="bringToFront" @focus="bringToFront">
            <header class="fm-panel-header" @mousedown.stop.prevent="startDrag">
                <div class="fm-title">移动衣橱</div>
                <div class="fm-header-actions">
                    <button class="header-btn" title="保存备份" @click.stop="saveBackup">💾</button>
                    <button class="header-btn" title="备份导入" @click.stop="importBackup">📂</button>
            ...
            <div class="fm-panel-body" :style="{ height: (panel.h - 56) + 'px' }">
                <aside class="fm-side" aria-hidden="false">
                    ...
                    <div class="fm-side-left-actions" role="group" aria-label="侧栏操作">
                        <button class="left-action-btn save-btn" title="保存角色到当前文件夹" @click.stop="saveCharacterToFolder">

                            <span class="la-text">Save</span>
                        </button>
                        <button class="left-action-btn import-btn" title="从 BCX 导入" @click.stop="importBCX">
                ...
                <main class="fm-main">
                    <FileManager :embedded="true" @close="requestClose" />
                </main>
...
function ensurePanelDefaults() {
    if (panel.value.x === null || panel.value.y === null) {
        panel.value.w = Math.min(1100, Math.round(hostWindow.innerWidth * 0.8))
        panel.value.h = Math.min(760, Math.round(hostWindow.innerHeight * 0.72))
        // 确保不超过视窗
...
async function applyImportedData(parsed) {
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            ...
            const name = prompt('请输入导入文件名：', defaultName)
            if (!name) {
                alert('已取消导入')
                return
            }
        ...
        if (parsed.fs && typeof parsed.fs === 'object') {
            ...
            try {
                ...
                fs.saveAll()
                alert('已从导入数据恢复文件系统（从 parsed.fs）')
                return
            } catch (e) {
        ...
        // 回退：无法识别的结构
        alert('导入完成，但未能识别数据结构以自动应用，请检查 JSON 内容。')
    } catch (e) {
...
async function importBCX() {
    const code = prompt('请粘贴 BCX code（base64 + LZString 压缩）：')
    if (!code) return
    try {
        ...
        alert('BCX 解码或解析失败，请检查输入（打开控制台查看详细错误）')
    }
}

/* ------------------- 现有功能（保持不变） ------------------- */

function saveCurrentToFolder() {
    if (!fs.activeItem || !fs.activeItem.data || !Array.isArray(fs.activeItem.data)) {
        alert('当前没有可保存的 active 数据')
        return
    }
    const defaultName = _defaultFilename('current')
    const name = prompt('保存当前 active 到当前文件夹，文件名：', defaultName)
    if (!name) return
    const file = {
        name: name,
        type: 'outfit',
...
function saveCharacterToFolder() {
    if (!fs.characterItem || !Array.isArray(fs.characterItem)) {
        alert('当前没有 character 数据可以保存')
        return
    }
    const defaultName = _defaultFilename('character')
    const name = prompt('保存 character 到当前文件夹，文件名：', defaultName)
    if (!name) return
    const file = {
        name: name,
        type: 'character',
...
.fm-panel-shell {
    ...
    user-select: none;
    transition: box-shadow .18s ease, transform .0s;
...
.fm-panel-body {
    ...
    overflow-x: hidden;
    overflow-y: hidden;
    min-height: 0; /* allow children to shrink for proper overflow handling */
}

/* left side preview area */
/* 使用灵活的宽度，同时保留一个合适的最小值，避免在小屏幕时撑出横向滚动 */
.fm-side {
    flex: 0 0 300px; /* 默认固定 300px 宽度 */
    max-width: 40%;
    min-width: 220px;
    height: auto;
...
.fm-main {
    ...
    background: transparent;
    border-radius: 8px;
    overflow-y: auto; /* 允许纵向滚动，但禁止横向滚动 */
    overflow-x: hidden;
