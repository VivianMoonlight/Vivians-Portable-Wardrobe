...
async function applyAsset(asset) {
  ...
  if (!canApply.value) {
    // use i18n message
    alert(t('assetSelector.alertNoReplaceMode'))
    return
  }
  ...
  try {
    ...
    if (!res) {
      ...
      alert(t('assetSelector.alertApplyFailed'))
      return
    }
...
async function onHoverAsset(asset) {
  ...
  try {
    ...
    store.renderer.renderPreviewWithItem(toRaw(store.mergedAppearanceData))
...
.asset-row {
  ...
  padding: 8px;
  border-radius: 0px;
  border: 0.5px solid rgba(233, 236, 240, 0.7);
  background: #fff;
...
.asset-row:hover {
  background: #fbfdff;
  border-color: rgba(120, 160, 215, 0.12);
}
...
.asset-card:hover {
  box-shadow: 0 0 8px rgba(140, 160, 210, 0.2);
}
